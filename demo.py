...
#        l.compute_word_weights()
        for i in range(0, e):
            qsn = raw_input()
            try:
                limit = int(raw_input('How many matches do you need?'))
            except:
                limit = int(raw_input('How many matches do you need?'))
            print "Matches for :", qsn.strip('\n')
            try:
                best_similar = l.calculate_similar(l.prepare_question(qsn), limit)
                temp = heapq.nlargest(limit, best_similar, key = best_similar.get)
                for ind, item in enumerate(temp):
                    print ind+1 , ". ", item, 'scored: ',best_similar[item]
            except:
                best_similar = ['Nothing except stopwords. Please try something else?', 0]
                print "Try some other question please. I cannot make a vector our of these words"
            # qsn = opener.readline()
            # best_similar = l.calculate_similar(l.prepare_question(qsn), 2)
            # print "Best Match for the question ",qsn, "is :", best_similar[0], 'scored: ',best_similar[1]
